package Algorithm.Algorithm25.Java.BOJ17472;
...
public class Main {
    ...
    // 섬 ai와 bi의 최단 거리 계산 (연결할 수 없으면 -1리턴)
    private static int getDistBetween(TreeSet<Point> ai, TreeSet<Point> bi){
        int res = INF;

        for (Point p : ai){
            for (int i = 0; i < 4 ; i++) {
                // 섬 ai에서 출발해 상하좌우 가까운 섬 좌표 찾기
                int nx = p.x + dx[i];
                int ny = p.y + dy[i];
                if (nx < 0 || nx >= N || ny < 0 || ny >= M) continue;
                if (graph[nx][ny] != 0) continue;   // 섬 내부면 continue
    ...
    public static void main(String[] args) throws IOException {
        ...
        int t = kruskal(n);
        System.out.println(root == 1 ? t : -1);     // 엣지가 있어도 root가 여러개면 모든 섬 연결 실패
...
* : 각 섬 외곽 points 간 바다 거리를 계산해서, 그게 2 이상일 경우만 고려해, 섬 간 최단 거리를 찾아내야 함
