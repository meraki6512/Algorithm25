...
def rotate(rl):
    ...
    if d[0] == 0:           # 원래 좌우우 방향으로 이동하고 있었을 때
        if rl == "D":       # 오른쪽으로 회전하하면
            d[0] = d[1]    
        else:               # 왼쪽으로 회전하면
            d[0] = -d[1]
    ...
    elif d[1] == 0:         # 원래 상하 방향으로 이동하고 있었을 때
        if rl == "D":       # 오른쪽으로 회전하면
            d[1] = -d[0]
        else:               # 왼쪽으로 회전하면
            d[1] = d[0]
...
while True:
    ...
    if nx <= 0 or nx > N or ny <= 0 or ny > N or baam[nx][ny] == 1:
        time += 1   # time = 0부터 세기 때문에 마지막에 ++ 해줘야 개수가 제대로 세짐
        break
...
# self-feedback
# row와 col 자꾸 바꿔서 생각하는 실수를 한다.. 주의.!: arr[x][y]면 x가 col, y가 row.
# 구현이 지저분해 보이기는 하니까 나중에 velog 정리할 때 다른 코드 찾아보고 읽어보자