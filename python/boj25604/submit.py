...
while que[0] or que[1]:
    ...
    if sumM != 0 and not nq:                        # 적재O 다음X (이 부분 생략 가능 (루프 2번))
        cur_time += 2 * T                           # 이동하고 다시 돌아옴
        ...
        cur_time += T                               # 바로 넘어가서 기다리기 (**)
        cur_pos ^= 1
    
    elif nq:                                        # 적재X 현재O 다음O
        if q[0][2] <= nq[0][2]:                     # 현재가 더 먼저 준비되면
            cur_time = q[0][2]                      # 현재부터
        
        elif q[0][2] > nq[0][2]:                    # 다음이 더 먼저 준비되면
            cur_time = max(cur_time + T, nq[0][2] + T) # 다음까지 기다린 뒤에 이동 (**주의): max 현재 시간에 이동시간 더한 것 vs 다음 준비시간에 이동시간 더한것
            cur_pos ^= 1
...
# 
# velog에 정리해두기
# 문제가 복잡해서 어려웠는데 참고할 코드가 없어서 더 힘들었다...